...
public class Customer extends NewPurchases{
	 ...
	 * @return
	 */
	public ArrayList<NewPurchases> findLoyals(ArrayList<Dbconnection> totalFees,ArrayList<NewPurchases> newoffered, ArrayList<NewPurchases> OldCustomers) {
		NewPurchases newf;
		...
		for (int i=0;i<getOldCustomers().size();i++) {
			...
			do {
				if ( NewPurchases.getOldCustomers().get(counter).getNewFees()==offered.get(i).getNewName()) {
					found=true;
				}
			...
			if(found ==false) {
				...
				if ( amount< totalFees.get(position).getMin_fees()) {
					newf=new NewPurchases(NewPurchases.getOldCustomers().get(counter).getNewName(),NewPurchases.getOldCustomers().get(counter).getNewMail(),NewPurchases.getOldCustomers().get(counter).getNewFees());
					newoffered.add(newf);
				}
	...
	public ArrayList<NewPurchases> findMoreOldCustomers() {
		...
		//It is a counter and it helps to reach every time the length of the NewCustomers.
		int j=0;
		
		//That's the primary length of the list NewCustomers before any customer be deleted.
		int lengthoriginal=NewPurchases.getNewCustomers().size();
		
		//A list that contains the position that a customer has appeared at the list NewCustomers.
		...
		//That's the float type of the String newFees that the NewCustomer list contains.
		float amount;
		NewPurchases newf;
		...
		do {
			...
			//Checking if the name already exists for 10times, meaning if that person has gotten more than 10 months purchases.
			do {
			...
			/*
			 * If the person has done more than 10 monthly purchases he has to be transfered into another list, called newoldcustomers 
			 * and he has to be deleted from the list NewCustomers.
			 */
			if(i>=10) {
				for(int a=0;a<=counter.size();a++) {
					amount=Float.parseFloat(NewPurchases.getNewCustomers().get(counter.get(a)).getNewFees());
					newobject=new NewPurchases(NewPurchases.getNewCustomers().get(j).getNewName(),NewPurchases.getNewCustomers().get(j).getNewMonth(),amount,NewPurchases.getNewCustomers().get(j).getNewMail());
